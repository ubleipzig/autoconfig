...
program
	...
	.option('-d, --defaults [value]', 'the folder with default overrides. they must follow the naming convention <site>.json', path.join(process.env.HOME, '.autoconfig/'))
...
function updateDefaults(site, defaults) {
	...
	fs.mkdir(program.defaults, '0700', function(err) {
		if (err.code !== 'EEXIST') deferred.reject(err);
...
function createConfigs(site, defaults, configs) {
	return Q.fcall(function() {
		var instanceConfigPath = path.join(program.basedir, site, site, program.instance);
		...
		return Q.all(_.map(configs, function (filePath) {
			...
			return configtools.create(options).then(function (options) {
				...
				try {
					var configurator = require('../lib/vufind2/' + options.configName);
				} catch (err) {
					var deferred = Q.defer();
					deferred.resolve('nothing to configure for ' + options.configName);
					return deferred.promise;
				}
				return configurator(options, defaults);
