...
var createConfigs = function (defaults, parentConfigs, parentLanguages) {
	...
	return Q.all(parentConfigs.map(function (filePath) {
		...
		return Q.all(parentLanguages.map(function (filePath) {
...
function createConfig(inheritedConfig, destConfig, templateVars) {
	...
	if (ext === '.ini') {
		...
		fs.createReadStream(inheritedConfig).pipe(
			fs.createWriteStream(destConfig, {
				flags: 'wx'
